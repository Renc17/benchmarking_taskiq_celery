...
class SQSBroker(AsyncBroker):
    ...
    def __init__(
        ...
        max_messages: int = 10, # capped at 10 as per SQS limits
        wait_time: int = 5,
        endpoint_url: str | None = None,
    ) -> None:
        super().__init__()
        ## TODO: support multiple queues in the future
    ...
    async def startup(self) -> None:
        """Starts up broker and initializes resources."""
        if not self.session:
            logger.error("Failed to create SQS session")
            return
        self.client: AioBaseClient = await self._exit_stack.enter_async_context(
            self.session.create_client(
                "sqs", region_name=self.aws_region, endpoint_url=self.endpoint_url
            )
        )
        await super().startup()

    async def shutdown(self) -> None:
        """Shuts down broker and cleans resources."""
                ...
                for msg in messages:
                    ...
                    yield AckableMessage(
                        ...
                        ack=lambda receipt_handle=msg[  # type: ignore[misc]
                            "ReceiptHandle"
                        ]: self._acknowledge_message(self.client, receipt_handle),
                    )
            except Exception as e:
